...
class Maximum(nn.Cell):
    ...
    def construct(self, a, b):
        """ forward """
        # a [49104, ] b [128, ]
        a = self.expand_dims(self.squeeze(a), 1) # [49104,]
        a = self.tile_op(a, (1, 128))  # [49104, 128]

        b = self.expand_dims(self.squeeze(b), 0) # [1, 128]
        b = self.tile_op(b, (49104, 1)) # [49104, 128]
...
class FocalLoss(nn.Cell):
    ...
    def construct(self, regressions, classifications, anchor, annotations):
        ...
        gt_loc = annotations[:, :, 0:4:1]  # (BS, 128, 4)
        gt_label = self.squeeze_dim2(annotations[:, :, 4:5:1])  # (BS, 128)
        ...
        IoU = self.stack(IoU)  # [12,49104,128]
        IoU_argmax, IoU_max = self.argmaxwithvalue(IoU)  # [BS,49104,1]

        IoU_argmax = self.squeeze_dim2(IoU_argmax)  # [BS,49104]
        gt_label_int = self.cast(gt_label, mindspore.int32)
        want_label = self.gatherD(gt_label_int, 1, IoU_argmax)  # [BS,49104]

        target = self.cast(self.onehot(want_label, classifications.shape[-1], self.on_value,
                                       self.off_value), mindspore.int32)   # [BS,49104,90]

        mask_greater = F.cast(self.not_op(self.less(IoU_max, 0.5)), mindspore.int32)  # [BS,49104,1]
        mask_greater_2 = self.cast(self.squeeze_dim2(mask_greater), mindspore.float32)

        positive_num = self.squeeze_dim1(self.reduce_sum(mask_greater_2, 1))  # [BS,]
        ...
        mask_less = self.tile(mask_less, (1, 1, classifications.shape[-1])) # [BS, 49104, 90]
        ...
        IoU_argmax_ex = self.expand_dims(IoU_argmax, -1)    # [BS, 49104, 1]
        IoU_argmax_loc = self.tile(IoU_argmax_ex, (1, 1, 4))   # [BS,49601,4]
        target_loc = self.gatherD(gt_loc, 1, IoU_argmax_loc)  # [BS,49601,4]
        mask_greater_loc = F.cast(self.greater(IoU_max, 0.5), mindspore.int32)   # [BS,49601,1]
        mask_greater_loc = self.tile(mask_greater_loc, (1, 1, 4))

        # [BS, 49104, 4]  "4": [x, y, w, h]
